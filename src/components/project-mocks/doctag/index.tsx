...
export function DoctagMock() {
  ...
  return (
    <div className="rounded-md bg-bg-dim p-4">
      <p className="mb-3 font-mono text-[12px] uppercase tracking-wide text-fg-muted">
      ...
      <ul className="flex flex-col gap-2">
        {documents.map((doc) => (
          <li key={doc.id} className="flex items-center justify-between gap-3 rounded bg-bg px-3 py-2">
            <span className="text-[13px] text-fg">{doc.name}</span>
            <button
              type="button"
              draggable
              ...
              className="cursor-grab rounded bg-lime-soft px-3 py-1 font-mono text-[13px] text-lime-deep active:cursor-grabbing motion-safe:transition-transform motion-safe:active:scale-95"
            >
              {doc.tag}
            </button>
          </li>
