  ...
  return (
    <header className="relative sticky top-0 z-40 border-b border-bg-dim bg-bg px-4 py-3">
      <div className="mx-auto flex w-full max-w-6xl items-center justify-between">
        ...
        <nav aria-label="Navegação principal" className="hidden gap-4 text-[15px] sm:flex">
          {NAV_ITEMS.map((item) => (
            <a
              key={item.id}
              href={`#${item.id}`}
        ...
        <div className="flex items-center gap-2">
          <ThemeToggle />
          <span
            className="rounded border border-fg-muted px-1.5 py-0.5 font-mono text-[13px] text-fg-muted"
